...
# Date: January 2026
# Works ONLY with Binance (Crypto)
# =============================================
...
def macd_trend(df):
    ...
    macd_line = exp1 - exp2    signal = macd_line.ewm(span=9).mean()
...
def quanta_analyze(symbol: str):
    # Only crypto via Binance
    df_m15 = get_binance_ohlcv(symbol, "15m")
    df_h1 = get_binance_ohlcv(symbol, "1h")
    df_h4 = get_binance_ohlcv(symbol, "4h")
    df_d1 = get_binance_ohlcv(symbol, "1d")
    ...
    macd_trends = {}
    for tf_name, df in [("M15", df_m15), ("H1", df_h1), ("H4", df_h4), ("D1", df_d1), ("W1", df_w1)]:        macd_trends[tf_name] = macd_trend(df) if df is not None else "NO_DATA"
...
if __name__ == "__main__":
    ...
    def home():
        ...
        <p>مثال: <a href='/report?symbol=ETHUSDT'>تحليل الإيثيريوم</a></p>
        """
